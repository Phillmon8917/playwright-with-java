@regression @guest @shard2
Feature: Agent Signup
    As a guest user
    I want to sign up as an agent
    So that I can access agent features on the platform

    Background:
        Given the home page is loaded
        And I navigate to the agent signup page

    @sanity
    Scenario: Successful agent signup
        Given a security check question is displayed
        When I solve the security check question
        And I fill the agent signup form with valid details
        Then the signup should be successful with no backend validation errors

    @sanity
    Scenario: First name field validation on agent signup
        When I submit the agent signup form with an empty first name
        Then a validation error should be shown for the first name field

    Scenario: Last name field validation on agent signup
        When I submit the agent signup form with an empty last name
        Then a validation error should be shown for the last name field

    Scenario: Email field validation on agent signup
        When I submit the agent signup form with an empty email
        Then a validation error should be shown for the email field

    @sanity
    Scenario: Password field validation on agent signup
        When I submit the agent signup form with an empty password
        Then a validation error should be shown for the password field

    @sanity
    Scenario: Confirm password field validation on agent signup
        When I submit the agent signup form with an empty confirm password
        Then a validation error should be shown for the confirm password field
